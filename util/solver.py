...
class Solver(object):
    ...
    def _reset_histories(self):
		...
		def _gaussian_distribution2d(self, out_mu_x, out_mu_y, out_sigma, out_corr, fix_data):
				...
				result = (fix_data.expand_as(out_mu_xy)  - out_mu_xy) 
				result = result ** 2
				result = result.sum(dim=2, keepdim=True)
				result = torch.squeeze(result,2)
				result = -.5 * result * torch.reciprocal(out_sigma ** 2)

				return (torch.exp(result) * torch.reciprocal(out_sigma ** 2)) * self.oneDivTwoPI
